...
<template>
  <v-dialog :model-value="modelValue" @update:model-value="emit('update:modelValue', $event)" max-width="600" persistent>
    <v-card class="person-edit-modal">
      <v-card-title class="person-edit-modal__title">
        <v-icon class="mr-2">mdi-pencil</v-icon>
        Editar Usuário
      </v-card-title>

      <v-card-text class="person-edit-modal__content">
        <v-form @submit.prevent="handleSubmit">
          ...
          <div class="person-edit-modal__regions-group">
            <label class="person-edit-modal__regions-label">Zonas Responsáveis *</label>
            <div class="person-edit-modal__regions-checkboxes">
              <v-checkbox
      ...
      <v-card-actions class="person-edit-modal__actions">
        <v-spacer></v-spacer>
        <v-btn variant="text" @click="closeModal" :disabled="loading"> Cancelar </v-btn>
        <v-btn color="primary" variant="flat" @click="handleSubmit" :loading="loading"> Atualizar </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>

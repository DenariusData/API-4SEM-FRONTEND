...
async function loadZonesData() {
  ...
  try {
    ...
    const promises = zonesData.value.map(async (zone) => {
      try {
        const response = start === end ? await getAllRoadsDataForDate(zone.regionId, start) : await getAllRoadsDataForRange(zone.regionId, start, end)
        let dataArray: any[] = []
...
async function loadHourlyDataForAllZones() {
  try {
    ...
    const promises = zonesData.value
      ...
      .map(async (zone) => {
        try {
          const response = await getVehiclesPerHourForRoad({ regionId: zone.regionId, roadId: zone.corridors[0].id, start, end })
        ...
        } catch { return { zone: zone.zone, data: Array(13).fill(0) } }
      })
  ...
  } catch { return getMockHourlyData() }
}
...
watch(activeDashboard, async (newDash) => { if (newDash === 'line') await createLineChart() })
...
const createBarChart = () => {
  ...
  barChartInstance = new Chart(barChartRef.value, {
    ...
    options: {
      ...
      scales: {
        y: { beginAtZero: true, max: chartMax, ticks: { stepSize: 500, font: { size: 12 }, color: '#7a7a7a' }, grid: { color: '#e0e0e0' } },
...
const createLineChart = async () => {
  ...
  lineChartInstance = new Chart(lineChartRef.value, {
    ...
    options: {
      ...
      plugins: {
        legend: { display: true, position: 'top', labels: { font: { size: 13, weight: 600 }, color: '#4d4d4d', padding: 15, usePointStyle: true, pointStyle: 'circle' } },
        tooltip: { backgroundColor: 'rgba(0,0,0,0.8)', titleFont: { size: 14, weight: 'bold' }, bodyFont: { size: 13 }, padding: 12, cornerRadius: 8, displayColors: true, callbacks: { label: (ctx) => `${ctx.dataset.label}: ${ctx.parsed.y} veículos` } }
      },
      scales: {
        y: { beginAtZero: true, max: chartMax, ticks: { stepSize: Math.ceil(chartMax / 5 / 100) * 100, font: { size: 12 }, color: '#7a7a7a', callback: (v) => v + ' veíc.' }, grid: { color: '#e0e0e0' } },
...
<template>
  <div class="modal-overlay" @click="closeDashboard">
    <div class="modal-content" @click.stop>
      ...
      <div class="dash-view">
        <div class="dashboard-tabs">
          <button :class="['tab-button', { active: activeDashboard === 'bar' }]" @click="switchDashboard('bar')">Dashboard dos Corredores Principais</button>
          <button :class="['tab-button', { active: activeDashboard === 'line' }]" @click="switchDashboard('line')">Evolução do Tráfego ao Longo do Dia</button>
        </div>
        ...
        <div v-if="activeDashboard === 'bar'" class="dashboard-content">
          <div class="zone-carousel">
            <button class="carousel-btn prev" @click="navigateZone(-1)" :disabled="filteredZones.length === 1">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M15 18L9 12L15 6" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
            ...
            <button class="carousel-btn next" @click="navigateZone(1)" :disabled="filteredZones.length === 1">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M9 18L15 12L9 6" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
          </div>
          ...
          <div v-else class="corridor-cards">
            <CorridorCard v-for="c in corridorData" :key="c.id" :name="c.name" :vehicles="c.vehicles" :speed="c.speed" />
          </div>
    ...
    <ComparisonZones v-if="showComparisonModal" :zones-data="zonesData" :filtered-zones="filteredZones" @close="showComparisonModal = false" />
  </div>
</template>
